...
function stockpoint( symbol , securityExchange , connurl , foucs , foucs_index ) {
        ...
        $.ajax({
                type: 'Get',
                url: connurl + 'api/market-data/k-bar/' + symbol ,
                dataType: "json",
                data: {
                        page: "0",
                        period: "m5",
                ...
                success: function(data) {
                        ...
                        for (var i=0 ; i < dataLength ; i++ ) 
                        {
                                ...
                                var index = i ;
                                ohlc.push([
                                        new Date(data[index].T).getTime(), // the date
                                        data[index].FP, // open
                                        data[index].HP, // high
                                        data[index].LP, // low
                                        data[index].P, // close
...
function fun_controller_istera_ApiSymbolInde()
{
            ...
            $.ajax({
                    ...
                    success: function(data) {
                            ...
                            $( "#controller_istera_ApiSymbolInde" ).unbind('change').bind( "change" , function(e) {

                                        stockpoint( $( "#controller_istera_ApiSymbolInde" ).val().toString() , 'SHSE' , website_ws_url , $("#container_stock") , 1 );
